...
with open(os.path.abspath('app/secrets/firebaseConfig.json'), 'r') as f:
...
def create_post(self, nome, cpf, senha):
  ...
  request = db.child('Users').get()
  try:
    for user in request:
        valor = db.child('Users').child(user.key()).child('CPF').get().val()
        res.append(valor)
  except:
    res = []
    
  try:        
    to_database = json.loads('{"Nome": 'f'{json.dumps(nome)}'', "CPF" : 'f'{json.dumps(cpf)}'', "Senha" : 'f'{json.dumps(senha)}''}')
    ...
    if nome == "":
      ...
      db.child('Users').push(to_database)
...
def get_post(self, cpf, senha):
  #request = requests.get(firebase_url + '?auth=' + auth_key)
  #res = json.dumps(request.json())   
  res =[]
  request = db.child('Users').get()
  try:
    for user in request:
        valor = db.child('Users').child(user.key()).get().val()
        if valor.get('CPF') == cpf:
            res.append(valor.get('Senha'))
  except:
    res = []      
  
  if (cpf != '' and senha != ''):
    if (len(cpf) >= 11) and (len(senha) >= 10 and senha in res):
...
def change_screen(self, nome, cpf, id):
    ...
    user = db.child("Users").get()
    for usuario in user.each():
        if usuario.val()['CPF'] in cpf2 and usuario.val()['Senha'] in id2:
        
            if f'{nome}' != "" and nome not in nome2:
...
def check(self, especialidade, data, paciente):
    ...
    for pacient in pacients:
        res2.append(pacient.val()['ID Paciente'])
    ...
    try:
        db.child('Schedule').child('Consulta').push(json.loads(to_database))
...
def checkout(self, med, data, paciente2):
    ...
    pacients = db.child('Pacients').get()
    for pacient in pacients:
        res.append(pacient.val()['ID Paciente'])

    meds = db.child('Meds').get()
    for med1 in meds:
        res2.append(med1.val()['ID medicamento'])
    ...
    if med not in res2:
        ...
        try:
            db.child('Schedule').child('Retirada').push(json.loads(to_database))
...
def create_post_meds(self, nome_med, quantidade, id_med):
  ...
  if nome_med == "":
    ...
    try:        
      ...
      db.child('Meds').push(json.loads(to_database))
...
def load_data(self, *args): 
  get_request = requests.get(f'{databaseURL}'f'Meds.json?apiKey='f'{apiKey}')
...
def create_post_pacient(self, nome2, cpf2, senha2):
    res =[]
    request = db.child('Pacients').get()
    for pacient in request.each():
        res.append(pacient.val()['CPF'])

  #request = requests.get(self.firebase_url + '?auth=' + self.auth_key)
  #res = json.dumps(request.json()) 
  
    try:        
        to_database = json.loads('{"Nome": 'f'{json.dumps(nome2)}'', "CPF" : 'f'{json.dumps(cpf2)}'', "ID Paciente" : 'f'{json.dumps(senha2)}''}')
        ...
        if len(cpf2) != 11:
            ...
            db.child('Pacients').push(to_database)
...
def on_save3(self, instance, value, date_range):
  self.ids.data2.text = f'{str(value)}'
  #f'{str(date_range[0])} - {str(date_range[-1])}'
...
def create_post_hour(self, cpf_funcionario, data2, horario):
    ...
    if cpf_funcionario not in res:
        ...
        try:        
            ...
            db.child("Duty").push(json.loads(to_database))
...
def load_data2(self, *args): 
  get_request = requests.get(f'{databaseURL}'f'Schedule/Consulta.json?apiKey='f'{apiKey}')
  consultas_data = json.loads(get_request.content.decode())
...
def load_data3(self, *args): 
  get_request = requests.get(f'{databaseURL}'f'Schedule/Retirada.json?apiKey='f'{apiKey}')
  consultas_data = json.loads(get_request.content.decode())
...
def load_data4(self, *args): 
  ...
  get_request = db.requests.get(f'{databaseURL}'f'Duty.json')
...
def load_data5(self, *args): 
  get_request = requests.get(f'{databaseURL}'f'Pacients.json?apiKey='f'{apiKey}')
